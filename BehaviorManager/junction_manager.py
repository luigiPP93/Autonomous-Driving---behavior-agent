...
class JunctionManager(BasicAgent):
    ...
    def run_step(self, local_planner) -> bool:
        ...
        direction = self._local_planner.target_road_option if self._local_planner.target_road_option is not None else RoadOption.LANEFOLLOW
        ...
        # SCENARIO 1: FRONT LEFT JUNCTION
            ...
            vehicle_front = self._get_ordered_vehicles(wp_front, 5)
            vehicle_left = self._get_ordered_vehicles(wp_left, 5)
        ...
        # SCENARIO 2: FRONT RIGHT JUNCTION
            ...
            vehicle_front = self._get_ordered_vehicles(wp_front, 5)
            vehicle_right = self._get_ordered_vehicles(wp_right, 5)
            print("vehicle_front: ", vehicle_front)
        ...
        # SCENARIO 3: LEFT RIGHT JUNCTION
            ...
            if direction == RoadOption.LEFT:
                if not JunctionManager.is_vehicle_going_straight(vehicle_left) and not JunctionManager.is_vehicle_going_straight(vehicle_right) and not JunctionManager.is_vehicle_turning_left(vehicle_right):
                    print("--- [JUNCTION] Ego vehicle is going on the left and no vehicles can block the junction")
                    return True
                elif JunctionManager.is_vehicle_turning_right(vehicle_left):
                    print("--- [JUNCTION] Ego vehicle is going on the left and the vehicle on the left is turning right")
                    return True
            elif direction == RoadOption.RIGHT:
                if not JunctionManager.is_vehicle_going_straight(vehicle_left):
                    print("--- [JUNCTION] Ego vehicle is going on the right and no vehicles can block the junction")
                    return True
        ...
        # SCENARIO 4: ALL JUNCTION (FRONT, LEFT, RIGHT)       
            ...
            vehicle_front = self._get_ordered_vehicles(wp_front, 15)
            vehicle_left = self._get_ordered_vehicles(wp_left, 15)
            vehicle_right = self._get_ordered_vehicles(wp_right, 15)
            print("vehicle_front: ", vehicle_front)
            print("vehicle_left: ", vehicle_left)
            print("vehicle_right: ", vehicle_right)
    ...
    def __junction_is_free(self, junction: carla.Junction) -> bool:
        """
        This function checks if the junction is free from vehicles. It returns True if the junction is free from vehicles, False otherwise.
        
            :param junction (carla.Junction): reference junction to check.
            
            :return bool: True if the junction is free from vehicles, False otherwise.
        """
        # Get the pivot point of the junction. This corresponds to the center of the junction.
        pivot = carla.Transform(junction.bounding_box.location, carla.Rotation())
        # Get the radius of the junction. This corresponds to the distance between the center of the junction and the corner of the junction.
        ...
        # Filter the vehicles in the junction
        vehicles_in_junction = [
            vehicle for vehicle in vehicles_in_junction
            if self._map.get_waypoint(vehicle.get_location()).junction_id == junction.id
        ]

        return True if not vehicles_in_junction else False
