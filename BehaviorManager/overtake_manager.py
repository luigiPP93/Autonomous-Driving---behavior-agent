...
class OvertakeManager(BasicAgent):
    ...
    def run_step(
        ...
        if next_ego_wp:
            wp_in_junction = next_ego_wp[0].is_junction
        else:
    ...
    def __get_distance_other_lane(self, actor: carla.Actor, max_distance: float = 30) -> float:
        ...
        vehicle_list = [
            ...
            if v.id != actor.id and v.id != self._vehicle.id and dist(v, actor) < max_distance and self._parked_vehicle(v)[1] \
