...
class BehaviorAgent(BasicAgent):
    ...
    def run_step(self, debug=False):
        ...
        if tw_state is True:
            ...
            if not self._overtake_manager.in_overtake and tw_distance < 5:
        ...
        if affected_by_stop_sign:
            ...
            if sign_distance < 2 and not self._junction_manager.in_junction:
                ...
                control = self._junction_manager.run_step(self._local_planner)
        ...
        if vehicle_state:
            ...
            if abs(ego_vehicle_wp.lane_id) == 1 and is_a_bicycle(vehicle.type_id):
    ...
    def static_obstacle_manager(self, waypoint : carla.Waypoint, static_element="*static.prop*", angle_interval=[0, 90]):
        ...
        obstacles_list = [o for o in obstacles_list if is_within_distance(o.get_transform(), self._vehicle.get_transform(), 15, angle_interval=angle_interval)]
    ...
    def collision_and_car_avoid_manager(self, waypoint : carla.Waypoint) -> tuple:
        ...
        vehicle_list = [v for v in vehicle_list if dist(v, waypoint) < 12 and v.id != self._vehicle.id]       
    ...
    def pedestrian_avoid_manager(self, waypoint : carla.Waypoint) -> tuple:
        ...
        walker_list = [w for w in walker_list if is_within_distance(w.get_transform(), self._vehicle.get_transform(), 12, angle_interval=[0, 90])]
    ...
    def __update_information(self, debug : bool = False) -> None:
        ...
        self._speed_limit -= 10 if precipitation_intensity >= 60 or preciptitation_deposits >= 60 else 0
        ...
        if precipitation_intensity >= 60 or preciptitation_deposits >= 60:
            print("[WORLD] It is raining!")
        print("[VEHICLE] Vehicle Speed: {0} - Speed Limit: {1}\n".format(self._speed, self._speed_limit))
